...
> [!WARNING]  
> The library presented in this post has changed named. Its name used to be `template-haskell-natural`. It is now named `th-builder`. 
...
This is what I have been working on during the last few weeks. I came up with [`th-builder`](https://github.com/Arthi-chaud/th-builder): a library that leverages lenses and the `State` monad to derive a monadic DSL for Template Haskell. 
...
The goal of this blog post is to present the ideas behind the library, as well as show some examples of meta-programs built using `th-builder`. 
...
To solve the first problem, I came up with a small library, [`extract-cons`](https://github.com/Arthi-chaud/th-builder), which allow _extracting_ all the constructors of a given type constructor into standalone datatypes:

```haskell
import Data.Constructor.Extract.TH

extractConstructorsOf 'Dec

-- ==> Generates the following ADTs:
data ClassD = MkClassD Cxt Name [TyVarBndr BndrVis] [FunDep] [Dec]
data InstanceD = MkInstanceD (Maybe Overlap) Cxt Type [Dec]
-- etc.  
```
...
The [`lens-adt`](https://github.com/Arthi-chaud/th-builder) package solves the second issue. It provides a simple TH utility function that generate a lens for each field of a datatype, using the `position` function from [`generic-lens`](https://hackage.haskell.org/package/generic-lens).

```haskell
makeADTLenses 'ClassD ["cxt", "name", "tyVarBndr", "funDep", "dec"]
makeADTLenses 'InstanceD ["overlap", "cxt", "ty", "dec"] 
```
...
All these lenses are generated and exposed by the [`th-lenses`](https://github.com/Arthi-chaud/th-builder) package. 

Good news: We now have everything we need to build ASTs imperatively!

```haskell
-- | Generate an instance for the given typeclass (Name) 
-- using the provided builder
genInstance :: Name -> StateT Q InstanceD () -> Q Dec
genInstance n builder = 
  execStateT builder (MkInstanceD Nothing [] (ConT n) []) 

setOverlap :: Overlap -> StateT Q InstanceD ()
setOverlap o = overlap ?= o

addInstanceArg :: Type -> StateT Q InstanceD ()
addInstanceArg tyArg = ty %= (`AppT` tyArg)
```

_Note_: This is not valid `th-builder` code, but has a similar feel to it.

### Ensuring correctness of the built AST with the `Builder` Monad

We could just use the lenses from `th-lenses` along with a `StateT Q` monad, and build the AST using the operators presented earlier. 
But we can go even further: `th-builder` provides another layer of safety to make sure that the produced AST is 'valid': for example, we need to make sure that the `LamE` has a `body`, that the last statement in a `DoE` is not a bind, etc. 
...
These definitions were so abstract that I could use them for all AST-building computations defined in `th-builder` [^2].
...
-- | Adds a pattern to the lambda and 
-- returns the bound name wrapped in `VarE`
...
`th-builder` comes with variants of the `GenExp` typeclass for `Dec`s, `Type`s and `Pat`s.

## Examples

To test that `th-builder` is actually usable, I spent some time reimplementing a couple of libraries that generate Haskell code using Template Haskell.
...
[`tuple-th`](https://hackage.haskell.org/package/tuple-th) is a small library that provides TH functions to manipulate functions. Internally, it relies on a thin layer of abstraction around TH, allowing the functions to be reasonably small. I'll leave you check out the original code. If the library were to be rewritten using `th-builder`, here is what it would look like.
...
If we were to reimplement the library's core function using `th-builder`, we would get the following:
...
Again, using `th-builder`, we could write the code generator like this:
...
genCase tyName = do
  ...
  where
    caseSignature = newSignature $ B.do
...
Some functions that use `Name`s (such as `getField`, to deconstruct a value using a constructor name) require them to be _visible_ (see [`reify`'s doc](https://hackage-content.haskell.org/package/template-haskell/docs/Language-Haskell-TH.html#v:reify)). For example, let's say you have this bit of code:

```haskell
f :: Q [Dec]
f = do
  myData <- genDec $ newData "MyData" $ newCon "MkData" $ addField [t|Int|]
  unpackData <- genDecs $ newFunc "unData" $ bodyFromExp $ B.do
    d <- arg
    value <- getField (mkName "MkData") 0 -- FAIL
    returns value
  return (myData : unpackData)
```

The call to `getField` will fail because the function relies on `reify` to get the constructor's argument count (and generate a valid pattern). One solution is to generate the two declarations in two separate splices. Another way would be to integrate a cross-`Builder` tracking of the declared data types, classes, etc. to allow retrieving information from their name.
...
Regarding build time, I would say it's reasonable: it takes 39 seconds to build `extract-cons`, `lens-adt`, `th-lenses` and `th-builder` (on a modern MacBook Pro, excluding dependencies, but they depend on `lens` and `mtl`).
...
It only relies on the AST definition through the lenses and the `Builder`s that rely on an intermediate state. So theoretically, only the latter need to be updated when a change in TH's API occurs (e.g. a new field in a constructor).
